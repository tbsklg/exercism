...
type Roll = Int
data Game = Game {frame :: Int, roll :: Roll, points :: Int}
  deriving (Eq, Show)

initialGame :: Game
initialGame = Game{frame = 1, roll = 0, points = 0}

score :: [Roll] -> Either BowlingError Int
score rolls = case validate rolls of
  Left e -> Left e
  Right rs -> play rs initialGame
 where
  play [] Game{frame = 11, points = p} = Right p
  play [s1, s2] Game{frame = 10, points = p}
    | s1 == 10 || s1 + s2 == 10 = Left IncompleteGame
    | otherwise = Right $ p + s1 + s2
  play [_, _, s3, _] Game{frame = 10, roll = r} =
    Left $ InvalidRoll{rollIndex = r + 3, rollValue = s3}
  play [10, s1, s2] Game{frame = 10, roll = r, points = p}
    | s1 /= 10, s1 + s2 > 10 = Left $ InvalidRoll{rollIndex = r + 2, rollValue = s2}
    | otherwise = Right $ p + 10 + s1 + s2
  play [s1, s2, s3] Game{frame = 10, roll = r, points = p}
    | s1 + s2 == 10 = Right $ p + 10 + s3
    | otherwise = Left $ InvalidRoll{rollIndex = r + 2, rollValue = s2}
  play (s1 : s2 : s3 : rs) state@Game{frame = f, roll = r, points = p}
  ...
  play (s1 : s2 : rs) state@Game{frame = f, roll = r, points = p}
